...
# 安装好 MySQL 后， 第一步：从开始菜单打开 'MySQL' ，并在任务栏中开启它
# 第二步：开始菜单打开 'MySQL  Command Line Client' ，并输入安装时你设置的 root 密码
...
async def test(loop):                      # *** 注意此处的密码填自己设的密码 ***
    await orm.create_pool(loop=loop, user='root', password='root', db='moe')
                                           # *** 注意此处的密码填自己设的密码 ***
    u = User(name='Test', email='test123@qq.com', passwd='1234567890', image='about:blank')
    print(u)
    await u.save()
